...
export const slice = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(createValidator.fulfilled, (state, action) => {
      ...
      state.backup_phone_number =
        action?.payload?.data?.data?.backup_phone_number || ""
      state.facebook_link = action?.payload?.data?.data?.facebook_link || ""
      state.instagram_username =
        action?.payload?.data?.data?.instagram_username || ""
      state.twitter_username =
        action?.payload?.data?.data?.twitter_username || ""
      state.personalized_message =
        action?.payload?.data?.data?.personalized_message || ""
    })
